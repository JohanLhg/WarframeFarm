...
public class AddPrimeDialog extends DialogFragment {
    private final Context context;
    private DialogAddItemBinding binding;
    private FarmDialogViewModel dialogViewModel;

    private SelectPrimeAdapter itemAdapter;
    ...
    public AddPrimeDialog(Context context) {
    ...
    public View onCreateView(@NonNull @NotNull LayoutInflater inflater, @Nullable @org.jetbrains.annotations.Nullable ViewGroup container, @Nullable @org.jetbrains.annotations.Nullable Bundle savedInstanceState) {
        ...
        itemAdapter = new SelectPrimeAdapter(context);
        recyclerItems.setAdapter(itemAdapter);
        recyclerItems.setLayoutManager(new LinearLayoutManager(context));

        buttonAddItems.setText(R.string.button_add_prime);
        buttonAddItems.setOnClickListener(v -> {
            List<String> primeNames = itemAdapter.getSelectedPrimeNames();
            List<PrimeComplete> primes = itemAdapter.getSelectedPrimes();
            dialogViewModel.addPrimes(primeNames, primes);
            dismiss();
        });
        ...
        //region Observers
        dialogViewModel.getRemainingPrimes().observe(getViewLifecycleOwner(), new Observer<List<PrimeComplete>>() {
            @Override
            public void onChanged(List<PrimeComplete> primes) {
                itemAdapter.updateItems(primes);
            }
        });
